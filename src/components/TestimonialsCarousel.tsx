...
export function TestimonialsCarousel() {
  ...
  return (
    <div className="relative w-full max-w-6xl flex items-center justify-center">
      {/* Left Button */}
      <button
        className="absolute left-0 z-10 bg-[#ffe3ef] text-[#f63436] rounded-full w-12 h-12 flex items-center justify-center shadow transition-all hover:bg-[#f63436] hover:text-white"
      ...
      <div className="flex flex-row gap-8 w-full justify-center">
        {visible.map((i, idx) => (
          <div
            key={i}
            className={`bg-white rounded-xl shadow-lg p-8 flex flex-col items-start w-full max-w-[340px] min-w-[260px] transition-all duration-500 animate-fade-in-up ${idx === 1 ? "scale-105 z-20" : "scale-95 z-10"}`}
          >
            <span className="text-[#f63436] text-3xl mb-4">&#8220;</span>
            <div className="text-zinc-700 mb-6">{testimonials[i].quote}</div>
      ...
      {/* Right Button */}
      <button
        className="absolute right-0 z-10 bg-[#ffe3ef] text-[#f63436] rounded-full w-12 h-12 flex items-center justify-center shadow transition-all hover:bg-[#f63436] hover:text-white"
